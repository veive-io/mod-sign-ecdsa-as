...
import * as modAbi from "../build/modsignecdsa-abi.json";
...
beforeAll(async () => {
    ...
    await localKoinos.deployContract(
        ...
        path.join(__dirname, "../build/release/ModSignEcdsa.wasm"),
        modAbi
    );
});
...
it("validate signature", async () => {
    ...
    expect(receipt.logs[0]).toStrictEqual(`[mod-sign-ecdsa] valid signature found`);
});