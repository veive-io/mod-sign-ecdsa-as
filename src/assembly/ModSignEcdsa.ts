...
export class ModSignEcdsa extends ModSign {
  ...
  is_valid_signature(args: modsign.is_valid_signature_args): modsign.is_valid_signature_result {
    ...
    if (args.signature!.length == 65) {
      ...
      if (Arrays.equal(signer, args.sender)) {
        System.log(`[mod-sign-ecdsa] valid signature found`);
        result.value = true;
      } else {
        System.log(`[mod-sign-ecdsa] no valid signature found`);
      }
    }
  ...
  manifest(): modsign.manifest {
    ...
    result.name = "ECDSA signature validator";
    result.description = "Module for validation of ECDSA signatures";
